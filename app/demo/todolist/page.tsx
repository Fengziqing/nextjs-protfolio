...
const TodoList = () => {
    ...
    function handleClickDone(targetId: number) {
        const newTask = task.map((item) =>
            item.id === targetId ? { ...item, isDone: true } : item,
        )
        setTask(newTask)
    }
    ...
    function handleClickEdit(targetId: number) {
        setTask(
            task.map((item) =>
                item.id === targetId ? { ...item, isEditing: true } : item,
            ),
        )
    }
    ...
    function saveChange(targetId: number) {
        ...
        const newTask = task.map((item) =>
            item.id === targetId
                ? { ...item, name: editValue, isDone: false, isEditing: false }
                : item,
        )
        setTask(newTask)
    }
    function cancelChange(targetId: number) {
        setTask(
            task.map((item) =>
                item.id === targetId ? { ...item, isEditing: false } : item,
            ),
        )
    }
    ...
    return (
        <div className=" block mx-12 py-12 px-2 justify-center min-h-[100vh]">
            ...
            <div className="flex content-center">
                <input
                    className="input border-solid border-2 border-gray-300"
                    type="text"
                    value={taskName}
                    onChange={handleTaskNameChange}
                ></input>
                <button className="btn btn-neutral ml-2" onClick={addTask}>
                    ADD
                </button>
            </div>
            <p className=" text-red-500 italic my-2">{wrongMessage}</p>
            <p className=" font-bold md:text-2xl text-xl my-6 dark:text-white">
                To Do List
            </p>
            <p className=" text-gray-400 italic md:text-sm text-xs my-5">
                *Please notice that this To-Do List will not save any data from
                you.*
            </p>
                    ...
                    return (
                        <li className=" flex my-3 break-words" key={item.id}>
                            {item.isEditing ? (
                                <>
                                    <input
                                        className=" input input-sm border-2 border-solid border-gray-300 text-sm font-normal"
                                        type="text"
                                        value={editValue}
                                        onChange={handleEditValue}
                                    ></input>
                                    <button
                                        className="btn btn-active ml-3 btn-sm"
                                        onClick={() => cancelChange(item.id)}
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        className="btn btn-neutral ml-3 btn-sm"
                                        onClick={() => saveChange(item.id)}
                                    >
                                        Save
                                    </button>
                                </>
            ...
            <button
                className="btn btn-success my-6"
                onClick={handleCleanAllDone}
            >
                Clean All Done
            </button>
        </div>
